...
class SymbolicModel():
    ...
    def generateReward(self, action_reward_dict, seed):
        ...
        np.save('../HTN_Planner/intrinsic_reward/FindTreasure/action_reward_' + seed +'.npy', action_reward_dict)
    ...
    def generatePlan(self, seed):
        ...
        _ = os.system('sh ../HTN_Planner/cmd_sh/FindTreasure_run_Seed_' + seed +'.sh')

        # update quality and plan
        if flag:  # flag means plan
            flag = False
            p = open('./out/FindTreasure_' + seed +'.out')  # result: plan
